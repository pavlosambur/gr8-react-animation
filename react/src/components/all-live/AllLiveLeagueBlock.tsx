...
const AllLiveLeagueBlock: React.FC<{
  ...
  return (
    <motion.div
      className={`flex flex-col w-full bg-[var(--background-main)] rounded-2xl mb-2 ${priority === 0 ? "order-0" : "order-1"}`}
      layout
    >
      <div className="flex w-full flex-col">
        <div
          className={`${
            matches.length >= 0
              ? `sticky top-0 border-b-[var(--divider-main)] border-b ${isOpen ? "rounded-t-2xl" : "rounded-2xl"}`
              : ""
