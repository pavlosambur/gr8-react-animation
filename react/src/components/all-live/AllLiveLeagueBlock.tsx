...
const AllLiveLeagueBlock: React.FC<{
    ...
    return (
        <motion.div className={`flex flex-col w-full bg-[var(--background-main)] rounded-2xl mb-2 ${priority === 0 ? "order-0" : "order-1"}`} layout>
            <div className="flex w-full flex-col">
                <div
                    className={`${
                        matches.length >= 0 ? `sticky top-0 border-b-[var(--divider-main)] border-b ${isOpen ? "rounded-t-2xl" : "rounded-2xl"}` : ""
                    } bg-[var(--background-main)] flex w-full px-4 h-10 items-center justify-between z-10`}
                >
                    <div className="flex w-full justify-between  cursor-pointer" onClick={handleToggleMatches}>
                        <div className="flex items-center">
                            <AllLiveLeagueTitle countryCode={matches[0].CE} leagueName={matches[0].LE} />
                        </div>
                        <motion.div
                    ...
                    <AllLiveFavoriteIconLeague size={4} leagueId={leagueId} onClick={handleFavoriteClick} />
                </div>
