...
export const getCountryCode = (countryName: string) => {
    ...
    const country = iso31661.find((entry) => entry.name && entry.name.toLowerCase() === countryName.toLowerCase());
...
export const fetchData = async (
    ...
    try {
        ...
        try {
            ...
            throw new SyntaxError(`Ошибка парсинга JSON: ${(jsonError as Error).message}`);
        }
...
export const getMatchWinnerOddFromJSON = (
    ...
    const foundItem = data.find((item) => item.G === targetG && item.T === targetT);
...
export const getMatchHandicapsAndTotalOddsFromJSON = (
    ...
    const foundItem = targetGroup.ME.find((item) => item.T === targetT && item.CE === 1);
